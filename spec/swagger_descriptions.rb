...
#### QRкод как картинка
Пример: ```#[SomeQrcode(asQrcode: true)]```
В списке подстановки должен быть передан "SomeQrcode": "http://www.somesite.ru". Этот код будет преобразован в картинку и вставлен в отчёт
#### Таблица
Пример:
```
...
Пример:
```
#[SomeName(asRepeatableTemplate: true, Template: template_name)]
```
Паратметр Template в данном случае обязателен. Он содержит имя ещё одного шаблона, который должен находиться в базе данных.
Этот шаблон будет повторён в выходных данных столько раз, сколько данных будет передано.
При этом, подставляемый шаблон может также иметь теги подстановки, которые будут заполнены на основании данных из списка подстановки.

В списке подстановки должен быть передан
```
"SomeName": [{field1: 1, field2: 1}, {field1: 1, field2: 1}, {field1: 1, field2: 1},
             {field1: 1, field2: 1}, {field1: 1, field2: 1}, {field1: 1, field2: 1}].
```
'
